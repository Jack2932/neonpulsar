...
def _setup_app_logging(app: Flask) -> None:
...
def _register_runtime_observability(app: Flask) -> None:
    if getattr(app, '_nc_runtime_observability_registered', False):
        return
    app._nc_runtime_observability_registered = True
    slow_ms = int(os.environ.get('SLOW_REQUEST_MS', '2500') or 2500)

    @app.before_request
    def _nc_runtime_before_request():
        try:
    ...
    @app.after_request
    def _nc_runtime_after_request(response):
        rid = getattr(g, 'request_id', '') or uuid.uuid4().hex[:12]
        ...
        try:
            ...
            if started is not None:
                ...
                if elapsed_ms >= slow_ms:
                    app.logger.warning('slow_request %s %s -> %s in %sms rid=%s', request.method, request.path, response.status_code, elapsed_ms, rid)
        except Exception:
            pass
        return response
...
def _register_health_routes(app: Flask) -> None:
    if getattr(app, '_nc_health_routes_registered', False):
        return
    app._nc_health_routes_registered = True

    def _build_health_payload(include_db: bool = False) -> dict:
        payload = {
            'ok': True,
            'service': 'neonchat',
        ...
        if include_db:
            ...
            try:
                ...
                db_ok = False
                payload['ok'] = False
                payload['db_error'] = str(exc)[:240]
            payload['db'] = 'ok' if db_ok else 'error'
        return payload

    @app.get('/healthz')
    def healthz():
        return jsonify(_build_health_payload(include_db=False)), 200

    @app.get('/readyz')
    def readyz():
        payload = _build_health_payload(include_db=True)
...
def create_app():
    ...
    app.config["TEMPLATES_AUTO_RELOAD"] = _env_flag("TEMPLATES_AUTO_RELOAD", False)
    app.config["SESSION_COOKIE_HTTPONLY"] = True
    app.config["SESSION_COOKIE_SAMESITE"] = os.environ.get("SESSION_COOKIE_SAMESITE", "Lax")
    app.config["SESSION_COOKIE_SECURE"] = _env_flag("SESSION_COOKIE_SECURE", os.environ.get("PREFERRED_URL_SCHEME", "https") == "https")
    ...
    app.config["ASSET_VERSION"] = os.environ.get("ASSET_VERSION", "20260403_chat_yabrowser_perf_v62")
    ...
    db_url = _normalize_database_url(os.environ.get("DATABASE_URL") or "")
    app.config["SQLALCHEMY_DATABASE_URI"] = db_url or ("sqlite:///" + os.path.join(BASE_DIR, "app.db"))
    ...
    _register_runtime_observability(app)
    _register_health_routes(app)
    try:
        app.jinja_env.globals.update(mask_email=_mask_email_for_ui, mask_phone=_mask_phone_for_ui)
    except Exception:
        pass
    ...
    with app.app_context():
        ...
        _ensure_db_columns()
        _ensure_billing_duration_columns()
...
def _ensure_db_columns() -> None:
    ...
    try:
        ...
        if "username_change_count" not in cols:
            db.session.execute(text("ALTER TABLE user ADD COLUMN username_change_count INTEGER DEFAULT 0"))
            db.session.commit()
            try:
                db.session.execute(text("UPDATE user SET username_change_count = 0 WHERE username_change_count IS NULL"))
                db.session.commit()
            except Exception:
                db.session.rollback()
            cols.append("username_change_count")

        if "email_change_count" not in cols:
            db.session.execute(text("ALTER TABLE user ADD COLUMN email_change_count INTEGER DEFAULT 0"))
            db.session.commit()
            try:
                db.session.execute(text("UPDATE user SET email_change_count = 0 WHERE email_change_count IS NULL"))
                db.session.commit()
            except Exception:
                db.session.rollback()
            cols.append("email_change_count")

        if "phone_change_count" not in cols:
            db.session.execute(text("ALTER TABLE user ADD COLUMN phone_change_count INTEGER DEFAULT 0"))
            db.session.commit()
            try:
                db.session.execute(text("UPDATE user SET phone_change_count = 0 WHERE phone_change_count IS NULL"))
                db.session.commit()
            except Exception:
                db.session.rollback()
            cols.append("phone_change_count")
...
# Ensure each non-DM channel has an invite code.
        ...
        # --- auth_session / auth_attempt migrations ---
        try:
            as_cols = [r[1] for r in db.session.execute(text("PRAGMA table_info(auth_session)")).fetchall()]
        except Exception:
            as_cols = []
        ...
        if not as_cols:
            ...
            try:
                if "token_hash" not in as_cols:
                    db.session.execute(text("ALTER TABLE auth_session ADD COLUMN token_hash VARCHAR(64)"))
                    db.session.commit()
                    as_cols.append("token_hash")
                if "ip_address" not in as_cols:
                    db.session.execute(text("ALTER TABLE auth_session ADD COLUMN ip_address VARCHAR(64)"))
                    db.session.commit()
                    as_cols.append("ip_address")
                if "created_at" not in as_cols:
                    db.session.execute(text("ALTER TABLE auth_session ADD COLUMN created_at DATETIME"))
                    db.session.commit()
                    as_cols.append("created_at")
                if "last_seen_at" not in as_cols:
                    db.session.execute(text("ALTER TABLE auth_session ADD COLUMN last_seen_at DATETIME"))
                    db.session.commit()
                    as_cols.append("last_seen_at")
                if "user_agent" not in as_cols:
                    db.session.execute(text("ALTER TABLE auth_session ADD COLUMN user_agent VARCHAR(512)"))
                    db.session.commit()
                    as_cols.append("user_agent")
            except Exception:
                try: db.session.rollback()
                except Exception: pass

        try:
            if "token_hash" in as_cols:
                rows = db.session.execute(text("SELECT id, token_hash, session_id FROM auth_session")).fetchall() if "session_id" in as_cols else db.session.execute(text("SELECT id, token_hash, NULL as session_id FROM auth_session")).fetchall()
                for sid, token_hash, legacy_session_id in rows:
                    next_hash = (token_hash or "").strip()
                    if not next_hash:
                        raw = (legacy_session_id or "").strip()
                        if raw:
                            next_hash = _nc_hash_session_token(raw)
                        else:
                            next_hash = _nc_hash_session_token(secrets.token_urlsafe(24))
                        db.session.execute(text("UPDATE auth_session SET token_hash = :th WHERE id = :id"), {"th": next_hash, "id": int(sid)})
                db.session.commit()
        except Exception:
            try: db.session.rollback()
            except Exception: pass
        ...
        try:
            if "ip" in as_cols and "ip_address" in as_cols:
                db.session.execute(text("UPDATE auth_session SET ip_address = COALESCE(NULLIF(ip_address, ''), ip) WHERE ip IS NOT NULL AND ip != ''"))
                db.session.commit()
        except Exception:
            try: db.session.rollback()
            except Exception: pass
        ...
        try:
            if "created_at" in as_cols:
                db.session.execute(text("UPDATE auth_session SET created_at = CURRENT_TIMESTAMP WHERE created_at IS NULL"))
                db.session.commit()
        except Exception:
            try: db.session.rollback()
            except Exception: pass
        ...
        try:
            if "last_seen_at" in as_cols:
                db.session.execute(text("UPDATE auth_session SET last_seen_at = COALESCE(last_seen_at, created_at, CURRENT_TIMESTAMP) WHERE last_seen_at IS NULL"))
                db.session.commit()
        except Exception:
            try: db.session.rollback()
            except Exception: pass
        ...
        try:
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_session_user_id ON auth_session(user_id)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_session_token_hash ON auth_session(token_hash)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_session_last_seen_at ON auth_session(last_seen_at)"))
            db.session.commit()
        except Exception:
            try: db.session.rollback()
            except Exception: pass
        ...
        try:
            ...
            """))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_attempt_user_id ON auth_attempt(user_id)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_attempt_session_id ON auth_attempt(session_id)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_attempt_status ON auth_attempt(status)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_attempt_login_value ON auth_attempt(login_value)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_attempt_ip_address ON auth_attempt(ip_address)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_auth_attempt_created_at ON auth_attempt(created_at)"))
            db.session.commit()
        except Exception:
            try: db.session.rollback()
            except Exception: pass
...
class User(db.Model, UserMixin):
    ...
    email_change_count = db.Column(db.Integer, default=0)
...
def _nc_auth_session_table_cols() -> set[str]:
    try:
        rows = db.session.execute(text("PRAGMA table_info(auth_session)")).fetchall()
...
NC_OWNER_USERNAME = "jaki2932"


def _nc_owner_user_id() -> int:
    try:
        u = User.query.filter(func.lower(User.username) == NC_OWNER_USERNAME.lower()).order_by(User.id.asc()).first()
        return int(u.id) if u else 1
    except Exception:
        return 1


def _nc_is_owner_user(user_or_id) -> bool:
    try:
        if hasattr(user_or_id, 'id'):
            uid = int(getattr(user_or_id, 'id', 0) or 0)
            uname = str(getattr(user_or_id, 'username', '') or '').strip().lower()
            if uname == NC_OWNER_USERNAME.lower():
                return True
            return uid > 0 and uid == _nc_owner_user_id()
        uid = int(user_or_id or 0)
        return uid > 0 and uid == _nc_owner_user_id()
    except Exception:
        return False
...
def _nc_same_client_fingerprint(user_agent: str | None, ip_address: str | None = None):
    try:
        meta = _nc_parse_user_agent(user_agent or '')
    except Exception:
        meta = {'browser': 'Browser', 'os': 'Unknown OS', 'device': 'Desktop'}
    return (
        (meta.get('browser') or '').strip().lower(),
        (meta.get('os') or '').strip().lower(),
        (meta.get('device') or '').strip().lower(),
        (ip_address or '').strip(),
    )


def _nc_find_reusable_auth_session(user_id: int, user_agent: str | None, ip_address: str | None):
    """Reuse one session row per browser/device/IP for the Devices page.

    Same PC + same browser should refresh the existing card instead of
    creating another active session row. Different browser keeps a separate row.
    """
    try:
        target_fp = _nc_same_client_fingerprint(user_agent, ip_address)
        rows = AuthSession.query.filter_by(user_id=int(user_id)).order_by(AuthSession.last_seen_at.desc(), AuthSession.id.desc()).all()
    except Exception:
        return None, []
    ...
    for row in rows:
        try:
            row_fp = _nc_same_client_fingerprint(getattr(row, 'user_agent', None), getattr(row, 'ip_address', None))
        except Exception:
            row_fp = ('', '', '', '')
        if row_fp == target_fp:
            matches.append(row)
    ...
    if matches:
        matches.sort(key=lambda r: (
            int(getattr(r, 'last_seen_at', None).timestamp()) if getattr(r, 'last_seen_at', None) else 0,
            int(getattr(r, 'created_at', None).timestamp()) if getattr(r, 'created_at', None) else 0,
...
def _nc_issue_auth_session(user_id: int) -> tuple[str, AuthSession] | tuple[None, None]:
    """Create or reuse an AuthSession row and return (raw_cookie_token, row).

    Same browser on the same PC should refresh the existing Devices card instead of
    creating a duplicate session row. Different browser keeps a separate row.
    Supports both the new schema and the user's legacy schema where auth_session
    still has NOT NULL session_id/ip columns.
    """
    try:
        ...
        ip = (_nc_client_ip() or "")[:64]
        cols = _nc_auth_session_table_cols()

        existing, duplicates = _nc_find_reusable_auth_session(int(user_id), ua, ip)
        if existing:
            ...
            if "session_id" in cols:
                try:
                    db.session.execute(
                        text("UPDATE auth_session SET session_id = :sid WHERE id = :id"),
                        {"sid": token[:80], "id": int(existing.id)},
                    )
                except Exception:
                    pass
        ...
        insert_cols = [c for c in (
            "user_id", "session_id", "token_hash", "created_at", "last_seen_at",
            "user_agent", "ip_address", "ip", "revoked_at"
        ) if c in cols]
        if not insert_cols:
            insert_cols = ["user_id", "token_hash", "created_at", "last_seen_at", "user_agent", "ip_address"]
        sql = "INSERT INTO auth_session (" + ", ".join(insert_cols) + ") VALUES (" + ", ".join(":" + c for c in insert_cols) + ")"
        db.session.execute(text(sql), {k: values.get(k) for k in insert_cols})
        db.session.commit()
        rec = AuthSession.query.filter_by(user_id=int(user_id), token_hash=th).order_by(AuthSession.id.desc()).first()
        return token, rec
    except Exception:
        try:
            db.session.rollback()
...
def _nc_ensure_current_auth_session(user_id: int) -> tuple[AuthSession | None, str | None]:
    """Make sure the current browser has its own auth_session row.

    This is a defensive helper for the Devices page too: even if an older browser
    cookie is reused in a new browser profile, we create a fresh row for the new
    browser instead of silently reusing the old Yandex/Chrome row.
    """
    try:
        uid = int(user_id or 0)
    except Exception:
    ...
    req_ip = (_nc_client_ip() or "")[:64]
    rec, tok = _nc_get_auth_session_for_request(uid)

    try:
        if not tok or not rec:
            nt, nrec = _nc_issue_auth_session(uid)
            if nt and nrec:
                g._nc_set_auth_cookie = nt
                try:
                    g._nc_auth_session_id = nrec.id
                except Exception:
                    pass
                return nrec, nt
            return rec, tok

        if not _nc_auth_session_same_client(rec.user_agent, req_ua):
            nt, nrec = _nc_issue_auth_session(uid)
            if nt and nrec:
                g._nc_set_auth_cookie = nt
                try:
        ...
        dirty = False
        if req_ua and (req_ua != (rec.user_agent or "")):
            rec.user_agent = req_ua
            dirty = True
        if req_ip and (req_ip != (rec.ip_address or "")):
            rec.ip_address = req_ip
            dirty = True
        now = datetime.utcnow()
        if (not rec.last_seen_at) or ((now - rec.last_seen_at).total_seconds() > 10):
...
def _nc_auth_session_same_client(stored_ua: str | None, current_ua: str | None) -> bool:
    """Returns True when the cookie is still used by the same browser/device family.

    Important for imported/shared cookies: if the same nc_auth_s cookie appears from a
    different browser (for example an old Yandex session copied into Chrome), we should
    not overwrite the existing row — we should issue a fresh session for the new browser
    so both devices/sessions are listed separately in settings.
    """
    a = (stored_ua or '').strip()
    b = (current_ua or '').strip()
    if not a or not b:
        return True
    if a == b:
        return True
    try:
        pa = _nc_parse_user_agent(a)
        pb = _nc_parse_user_agent(b)
    except Exception:
        return a == b
    return (
        (pa.get('browser') or '') == (pb.get('browser') or '')
        and (pa.get('os') or '') == (pb.get('os') or '')
        and (pa.get('device') or '') == (pb.get('device') or '')
    )


def _nc_compact_auth_sessions_for_view(rows, current_hash: str | None = None):
    """Keep one visible session card per browser/device/IP fingerprint.

    Same PC + same browser should look like one session whose dates refresh,
    not a stack of near-identical cards. If old duplicates already exist in the DB,
    the Devices page keeps only the best row (prefer current session, otherwise the
    most recent one).
    """
    items = list(rows or [])
    if len(items) < 2:
    ...
    for row in items:
        fp = (
            ...
            *_nc_same_client_fingerprint(getattr(row, 'user_agent', None), getattr(row, 'ip_address', None)),
        )
    ...
    for rows2 in groups.values():
        if len(rows2) == 1:
            compact.append(rows2[0])
            continue

        def _score(r):
            is_current = 1 if (current_hash and getattr(r, 'token_hash', None) == current_hash) else 0
            return (
                is_current,
                _ts(getattr(r, 'last_seen_at', None)),
                _ts(getattr(r, 'created_at', None)),
                int(getattr(r, 'id', 0) or 0),
            )

        compact.append(sorted(rows2, key=_score, reverse=True)[0])

    compact.sort(key=lambda r: (_ts(getattr(r, 'last_seen_at', None)), int(getattr(r, 'id', 0) or 0)), reverse=True)
    return compact
...
class UserSubscription(db.Model):
    ...
    renew_months = db.Column(db.Integer, default=1)
...
def _nc_login_manager_unauthorized():
    ...
    if wants_json:
        try:
            login_url = url_for("login", next=path)
        except Exception:
            login_url = "/login"
        return jsonify({"ok": False, "error": "unauthorized", "login_url": login_url}), 401
    return redirect(url_for("login", next=path or request.full_path or "/app"))
...
def _ensure_billing_duration_columns() -> None:
    """Lightweight SQLite migration for subscription duration fields."""
    try:
        sub_cols = [r[1] for r in db.session.execute(text("PRAGMA table_info(user_subscription)")).fetchall()]
        if "renew_months" not in sub_cols:
            db.session.execute(text("ALTER TABLE user_subscription ADD COLUMN renew_months INTEGER DEFAULT 1"))
            db.session.commit()
            db.session.execute(text("UPDATE user_subscription SET renew_months = 1 WHERE renew_months IS NULL OR renew_months < 1"))
            db.session.commit()
    except Exception:
        try: db.session.rollback()
        except Exception: pass
    try:
        gift_cols = [r[1] for r in db.session.execute(text("PRAGMA table_info(subscription_gift)")).fetchall()]
        if "duration_months" not in gift_cols:
            db.session.execute(text("ALTER TABLE subscription_gift ADD COLUMN duration_months INTEGER DEFAULT 1"))
            db.session.commit()
            db.session.execute(text("UPDATE subscription_gift SET duration_months = 1 WHERE duration_months IS NULL OR duration_months < 1"))
            db.session.commit()
    except Exception:
        try: db.session.rollback()
        except Exception: pass
...
def _billing_clamp_months(value, default: int = 1) -> int:
    try:
        months = int(value)
    except Exception:
        months = int(default or 1)
    if months < 1:
        months = 1
    if months > 12:
        months = 12
...
def _billing_plan_rank(plan_or_code) -> int:
    code = getattr(plan_or_code, 'code', plan_or_code)
    try:
        key = str(code or 'free').strip().lower()
    except Exception:
        key = 'free'
    return {'free': 0, 'plus': 1, 'pro': 2}.get(key, 0)


def _billing_auto_renew_subscription(sub: UserSubscription, plan: SubscriptionPlan) -> tuple[bool, UserSubscription | None, SubscriptionPlan | None]:
    if not sub or not plan:
        return False, sub, plan
    try:
        provider = str(getattr(sub, 'provider', 'mock') or 'mock').strip().lower()
    except Exception:
        provider = 'mock'
    if bool(getattr(sub, 'cancel_at_period_end', False)) or provider == 'gift':
        return False, sub, plan
    now = datetime.utcnow()
    period_days = int(getattr(plan, 'period_days', 30) or 30)
    renew_months = _billing_clamp_months(getattr(sub, 'renew_months', 1), 1)
    period_days_total = max(1, period_days * renew_months)
    start_at = getattr(sub, 'current_period_end', None) or now
    end_at = start_at + timedelta(days=period_days_total)
    while end_at <= now:
        start_at = end_at
        end_at = end_at + timedelta(days=period_days_total)
    try:
        sub.status = 'active'
        sub.current_period_start = start_at
        sub.current_period_end = end_at
        sub.renew_months = renew_months
        sub.canceled_at = None
        sub.ended_at = None
        db.session.add(BillingPayment(
            user_id=int(getattr(sub, 'user_id', 0) or 0),
            subscription_id=int(getattr(sub, 'id', 0) or 0),
            plan_id=int(getattr(plan, 'id', 0) or 0),
            provider=(provider or 'mock'),
            provider_payment_id=f"{provider or 'mock'}_renew_{int(getattr(sub, 'user_id', 0) or 0)}_{int(now.timestamp())}",
            amount_minor=int((getattr(plan, 'price_minor', 0) or 0) * renew_months),
            currency=(getattr(plan, 'currency', 'RUB') or 'RUB'),
            status='succeeded',
            payload_json=json.dumps({'type': 'renewal', 'plan_code': getattr(plan, 'code', ''), 'renew_months': renew_months}, ensure_ascii=False),
            paid_at=now,
        ))
        db.session.commit()
...
def _billing_get_active_subscription(user_id: int) -> tuple[UserSubscription | None, SubscriptionPlan | None]:
    ...
    plan = db.session.get(SubscriptionPlan, int(sub.plan_id)) if sub and getattr(sub, 'plan_id', None) else None
    if sub and sub.current_period_end and sub.current_period_end <= now:
        renewed = False
        if plan and not bool(getattr(sub, 'cancel_at_period_end', False)):
            renewed, sub, plan = _billing_auto_renew_subscription(sub, plan)
        if sub and renewed:
            return sub, plan
        try:
            sub.status = "expired"
...
def _billing_summary_for_user(user_id: int, include_payments: bool = False) -> dict:
    ...
    if _nc_is_owner_user(int(user_id)):
        try:
            owner_plan = SubscriptionPlan.query.filter_by(code='pro').first()
            if owner_plan:
                plan = owner_plan
                sub = sub or None
        except Exception:
...
def _gift_public(g: 'SubscriptionGift', include_code: bool = False) -> dict:
    ...
    out = {
        ...
        "duration_months": _billing_clamp_months(getattr(g, 'duration_months', 1), 1),
...
def _billing_apply_gift_to_user(user_id: int, plan: SubscriptionPlan, gift: 'SubscriptionGift') -> None:
    ...
    if period_days < 1:
        period_days = 30
    duration_months = _billing_clamp_months(getattr(gift, 'duration_months', 1), 1)
    ...
    new_end = base + timedelta(days=period_days * duration_months)
    ...
    if sub:
        ...
        sub.renew_months = duration_months
...
def _billing_feature_int(user_id: int, feature_key: str, default: int = 0) -> int:
    try:
        if _nc_is_owner_user(int(user_id)):
            if str(feature_key) == 'max_upload_mb':
                return 500
...
def _billing_activate_plan_and_payment(
    ...
    *,
    ...
    renew_months: int | None = None,
) -> dict:
    """Activate a user plan and mark payment succeeded (idempotent by provider_payment_id)."""
    now = datetime.utcnow()
    period_days = int(getattr(plan, "period_days", 30) or 30)
    duration_months = _billing_clamp_months(duration_months, 1)
    renew_months = _billing_clamp_months(renew_months if renew_months is not None else duration_months, duration_months)
    current_end = now + timedelta(days=max(1, period_days * duration_months))
    ...
    if not pay:
        pay = BillingPayment(
            ...
            amount_minor=int(amount_minor if amount_minor is not None else ((getattr(plan, "price_minor", 0) or 0) * duration_months)),
            currency=(currency or getattr(plan, "currency", "RUB") or "RUB"),
            status="succeeded",
            payload_json=json.dumps(payload or {"plan_code": plan.code, "provider": provider, "duration_months": duration_months, "renew_months": renew_months}, ensure_ascii=False),
            paid_at=now,
        )
        db.session.add(pay)
    else:
        ...
        pay.amount_minor = int(amount_minor if amount_minor is not None else ((getattr(plan, "price_minor", 0) or 0) * duration_months or pay.amount_minor or 0))
...
def _billing_create_checkout_for_user(user: User, plan: SubscriptionPlan, duration_months: int = 1) -> dict:
    provider = _billing_provider_name()
    ...
    if provider == "mock":
        billing = _billing_activate_plan_and_payment(
            ...
            amount_minor=int((getattr(plan, "price_minor", 0) or 0) * duration_months),
            currency=(getattr(plan, "currency", "RUB") or "RUB"),
            payload={"mode": "mock_checkout", "plan_code": plan.code, "duration_months": duration_months, "renew_months": duration_months},
            duration_months=duration_months,
            renew_months=duration_months,
        )
    ...
    if provider == "yookassa":
        ...
        body = {
            "amount": {
                "value": f"{((int(getattr(plan, 'price_minor', 0) or 0) * duration_months) / 100):.2f}",
                "currency": (getattr(plan, "currency", "RUB") or "RUB").upper(),
            },
            "capture": True,
            "confirmation": {"type": "redirect", "return_url": return_url},
            "description": f"Neon Chat {plan.name}",
            "metadata": {"user_id": str(int(user.id)), "plan_code": str(plan.code), "duration_months": str(duration_months)},
        }
        idem = uuid.uuid4().hex
...
def register_routes(app: Flask):
    ...
    def _nc_auth_sessions_guard():
        ...
        try:
            ...
            rec, tok = _nc_ensure_current_auth_session(uid)

            # Cookie exists but DB row still cannot be restored -> force logout
            if request.cookies.get(AUTH_SESSION_COOKIE) and not rec:
                try:
                    logout_user()
                except Exception:
                    pass
            ...
            try:
                g._nc_auth_session_id = rec.id if rec else None
            except Exception:
                pass
        except Exception:
            pass
...
def _nc_admin_revoke_user_sessions(user_id: int) -> int:
    try:
        deleted = AuthSession.query.filter_by(user_id=int(user_id)).delete(synchronize_session=False)
        db.session.commit()
...
def api_auth_audit():
    ...
    current_hash = None
    try:
        current_rec, tok = _nc_ensure_current_auth_session(uid)
        if tok:
            current_hash = _nc_hash_session_token(tok)
        elif current_rec and getattr(current_rec, 'token_hash', None):
            current_hash = current_rec.token_hash
    except Exception:
        current_hash = None
    ...
    try:
        ...
        sessions_rows = _nc_compact_auth_sessions_for_view(sessions_rows, current_hash=current_hash)
    except Exception:
        sessions_rows = []
...
def admin_support():
    ...
    try:
        ...
        if staff_profile is None and (_nc_auth_is_admin_auditor('support') or _nc_is_owner_user(current_user)):
...
def api_user_profile(user_id: int):
    ...
    badges = []
    try:
        if _nc_is_owner_user(u):
            badges.append({"key": "owner", "label": "Владелец Neon Pulsar"})
            badges.append({"key": "admin", "label": "ADMIN"})
        elif _nc_admin_permissions_for(int(u.id)):
...
def api_billing_gifts():
    ...
    return jsonify({
        'given': [_gift_public(g, include_code=False) for g in given],
...
def api_billing_gifts_create():
    ...
    duration_months = _billing_clamp_months(data.get("duration_months"), 1)
    ...
    if not to_uid:
        return jsonify({'error': 'Нужно выбрать друга для подарка'}), 400
    if int(to_uid) == int(current_user.id):
        return jsonify({'error': 'Нельзя подарить подписку самому себе'}), 400
...
@app.route("/api/billing/gifts/<int:gift_id>/claim", methods=["POST"])
@login_required
def api_billing_gifts_claim(gift_id: int):
    gift = db.session.get(SubscriptionGift, int(gift_id))
    if not gift:
        return jsonify({'error': 'gift_not_found'}), 404
    now = datetime.utcnow()
    if getattr(gift, 'status', 'active') != 'active':
    ...
    if gift.expires_at and gift.expires_at <= now:
        try:
            ...
            db.session.rollback()
        return jsonify({'error': 'gift_expired'}), 400
    if gift.to_user_id and int(gift.to_user_id) != int(current_user.id):
        return jsonify({'error': 'gift_not_for_you'}), 403
    plan = db.session.get(SubscriptionPlan, int(gift.plan_id)) if getattr(gift, 'plan_id', None) else None
    if not plan:
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'gift_claim_failed'}), 500
    try:
        if int(gift.from_user_id or 0):
            _emit_to_user(int(gift.from_user_id), 'gift_redeemed', {
                'gift_id': int(gift.id),
                'redeemed_by': int(current_user.id),
                'redeemed_by_username': current_user.username,
            })
    except Exception:
        pass
    return jsonify({'success': True, 'gift': _gift_public(gift, include_code=False), 'billing': _billing_summary_for_user(int(current_user.id), include_payments=False)})
...
def api_billing_subscribe():
    ...
    active_sub, active_plan = _billing_get_active_subscription(int(current_user.id))
    if active_sub and active_plan and _billing_plan_rank(plan) < _billing_plan_rank(active_plan):
        return jsonify({
            "error": "Нельзя перейти на подписку ниже, пока текущий период ещё активен",
            "error_code": "downgrade_not_allowed",
            "current_plan": getattr(active_plan, 'code', 'free'),
            "target_plan": getattr(plan, 'code', 'free'),
            "cancel_at_period_end": bool(getattr(active_sub, 'cancel_at_period_end', False)),
            "current_period_end": _iso_z(getattr(active_sub, 'current_period_end', None)) if getattr(active_sub, 'current_period_end', None) else None,
        }), 409

    # Choosing Free is only available when there is no active paid period.
    if plan.code == "free":
        return jsonify({"success": True, "billing": _billing_summary_for_user(int(current_user.id), include_payments=True)})
    ...
    try:
        billing = _billing_activate_plan_and_payment(
            ...
            amount_minor=int((getattr(plan, "price_minor", 0) or 0) * duration_months),
            currency=(getattr(plan, "currency", "RUB") or "RUB"),
            payload={"mode": "mock", "plan_code": plan.code, "duration_months": duration_months, "renew_months": duration_months},
            duration_months=duration_months,
            renew_months=duration_months,
        )
    except Exception:
...
def api_billing_resume():
    ...
    if not sub or not plan or plan.code == "free":
        return jsonify({"error": "Нет активной подписки"}), 400
    renew_months = _billing_clamp_months(data.get("renew_months") if isinstance(data, dict) else None, getattr(sub, 'renew_months', 1) or 1)
    try:
        sub.cancel_at_period_end = False
        sub.renew_months = renew_months
        sub.canceled_at = None
...
def api_billing_checkout_create():
    ...
    try:
        out = _billing_create_checkout_for_user(current_user, plan, duration_months=duration_months)
...
def api_billing_webhook(provider):
    ...
    if provider == "yookassa":
        ...
        meta_duration_months = _billing_clamp_months(meta.get("duration_months"), 1)
        ...
        if not pay and meta_user_id:
            ...
            try:
                ...
                amount_minor = int((getattr(plan_for_pending, "price_minor", 0) or 0) * meta_duration_months)
            pay = BillingPayment(
        ...
        if event == "payment.succeeded" or status == "succeeded":
            ...
            try:
                ...
                amount_minor = int((getattr(plan, "price_minor", 0) or 0) * meta_duration_months)
            try:
                billing = _billing_activate_plan_and_payment(
                    int(meta_user_id),
                    plan,
                    ...
                    payload=payload,
                    duration_months=meta_duration_months,
                    renew_months=meta_duration_months,
                )
            except Exception as e:
                db.session.rollback()
...
def _mask_email_for_ui(email: str | None) -> str:
    raw = str(email or '').strip()
    if not raw:
        return 'Не добавлен'
    if '@' not in raw:
        return '*****'
    domain = raw[raw.index('@'):]
    return '*****' + domain


def _normalize_phone_for_storage(value: str | None) -> str:
    ...
    if raw.startswith('00'):
        raw = '+' + raw[2:]
    if raw.startswith('8') and len(''.join(ch for ch in raw if ch.isdigit())) == 11:
        digits = ''.join(ch for ch in raw if ch.isdigit())
        raw = '+7' + digits[1:]
    elif raw.startswith('7') and not raw.startswith('+') and len(''.join(ch for ch in raw if ch.isdigit())) == 11:
        digits = ''.join(ch for ch in raw if ch.isdigit())
        raw = '+' + digits
...
def _validate_phone_for_ui(value: str | None) -> tuple[bool, str, str]:
    normalized = _normalize_phone_for_storage(value)
    if not normalized:
        return False, 'Введите новый номер телефона.', ''
    if not normalized.startswith('+'):
        return False, 'Номер должен начинаться с + и кода страны.', ''
    digits = ''.join(ch for ch in normalized if ch.isdigit())
    if not digits:
        return False, 'Номер должен содержать цифры.', ''

    rules = [
        ('375', 12, 12, 'Для Беларуси нужен номер в формате +375XXXXXXXXX.'),
        ('380', 12, 12, 'Для Украины нужен номер в формате +380XXXXXXXXX.'),
        ('7', 11, 11, 'Для России и Казахстана нужен номер в формате +7XXXXXXXXXX.'),
        ('1', 11, 11, 'Для США и Канады нужен номер в формате +1XXXXXXXXXX.'),
        ('44', 12, 12, 'Для Великобритании нужен номер в формате +44XXXXXXXXXX.'),
        ('33', 11, 11, 'Для Франции нужен номер в формате +33XXXXXXXXX.'),
        ('34', 11, 11, 'Для Испании нужен номер в формате +34XXXXXXXXX.'),
        ('49', 10, 13, 'Для Германии нужен корректный номер с кодом +49.'),
        ('39', 10, 13, 'Для Италии нужен корректный номер с кодом +39.'),
        ('86', 13, 13, 'Для Китая нужен номер в формате +86XXXXXXXXXXX.'),
        ('91', 12, 12, 'Для Индии нужен номер в формате +91XXXXXXXXXX.'),
    ]
    for code, min_len, max_len, err in rules:
        if digits.startswith(code):
            if len(digits) < min_len or len(digits) > max_len:
                return False, err, normalized
            return True, '', normalized
    if len(digits) < 8 or len(digits) > 15:
        return False, 'Номер должен быть в международном формате: + и от 8 до 15 цифр.', normalized
    return True, '', normalized


def _mask_phone_for_ui(phone: str | None) -> str:
    raw = _normalize_phone_for_storage(phone)
    if not raw:
        return 'Не добавлен'
    digits = ''.join(ch for ch in raw if ch.isdigit())
    if not digits:
    ...
    hidden_len = max(0, len(digits) - len(keep))
    return '+' + ('*' * hidden_len) + keep


def _norm_email_value(value: str | None) -> str:
    return str(value or '').strip().lower()


def _norm_phone_value(value: str | None) -> str:
    return _normalize_phone_for_storage(value)

def _account_limits_payload(user: User) -> dict:
    username_count = int(getattr(user, 'username_change_count', 0) or 0)
    email_count = int(getattr(user, 'email_change_count', 0) or 0)
    phone_count = int(getattr(user, 'phone_change_count', 0) or 0)
    return {
        'username_change_count': username_count,
        'username_change_allowed': max(0, 1 - username_count),
        'email_change_count': email_count,
        'email_change_allowed': max(0, 2 - email_count),
        'phone_change_count': phone_count,
        'phone_change_allowed': max(0, 2 - phone_count),
        'email_masked': _mask_email_for_ui(getattr(user, 'email', None)),
        'phone_masked': _mask_phone_for_ui(getattr(user, 'phone', None)),
    }
...
def api_account_update_username():
    v = _json_value("value")
    current_username = str(current_user.username or '').strip()
    if not v or len(v) < 3:
        return jsonify({"error": "Имя должно быть от 3 символов."}), 400
    if len(v) > 32:
        return jsonify({"error": "Слишком длинно (макс 32)."}), 400
    if v == current_username:
        payload = {"success": True, "username": current_user.username}
        payload.update(_account_limits_payload(current_user))
        return jsonify(payload)
    if int(getattr(current_user, 'username_change_count', 0) or 0) >= 1:
        return jsonify({"error": "Вы уже использовали свой единственный раз для изменения постоянного имени пользователя."}), 400
    ...
    try:
        ...
        current_user.username_change_count = int(getattr(current_user, 'username_change_count', 0) or 0) + 1
        try:
...
def api_account_update_email():
    data = request.get_json(silent=True) or {}
    v = str(data.get("value") or '').strip()
    password = str(data.get("password") or '')
    current_value = str(data.get("current_value") or '').strip()
    current_email = str(getattr(current_user, 'email', None) or '').strip()
    current_norm = _norm_email_value(current_email)
    new_norm = _norm_email_value(v)

    if not password or not current_user.check_password(password):
        return jsonify({"error": "Введите правильный пароль."}), 400
    if not v:
        return jsonify({"error": "Введите новую почту."}), 400
    if ("@" not in v) or ("." not in v.split("@")[-1]):
        return jsonify({"error": "Неверный email."}), 400
    if current_norm:
        if not current_value:
            return jsonify({"error": "Введите текущую привязанную почту для подтверждения."}), 400
        if _norm_email_value(current_value) != current_norm:
            return jsonify({"error": "Текущая почта указана неверно."}), 400
    if new_norm == current_norm:
        payload = {"success": True, "email": current_user.email or "", "email_masked": _mask_email_for_ui(current_user.email)}
        payload.update(_account_limits_payload(current_user))
        return jsonify(payload)
    if int(getattr(current_user, 'email_change_count', 0) or 0) >= 2:
        return jsonify({"error": "Лимит перепривязки почты исчерпан."}), 400
    try:
        existing = User.query.filter(func.lower(User.email) == new_norm, User.id != current_user.id).first()
    except Exception:
        existing = User.query.filter(User.email == v, User.id != current_user.id).first()
    if existing:
        return jsonify({"error": "Эта почта уже используется другим аккаунтом."}), 400
    try:
        current_user.email = v
        current_user.email_verified = False
        current_user.email_change_count = int(getattr(current_user, 'email_change_count', 0) or 0) + 1
        db.session.commit()
    except Exception:
        db.session.rollback()
        return jsonify({"error": "Не удалось сохранить."}), 500
...
def api_account_update_phone():
    ...
    password = str(data.get("password") or '')
    current_value = str(data.get("current_value") or '').strip()
    current_phone = str(getattr(current_user, 'phone', None) or '').strip()
    current_norm = _norm_phone_value(current_phone)
    ok_phone, phone_error, new_norm = _validate_phone_for_ui(v)
    ...
    if current_norm:
        if not current_value:
            return jsonify({"error": "Введите текущий привязанный номер для подтверждения."}), 400
        if _norm_phone_value(current_value) != current_norm:
            return jsonify({"error": "Текущий номер указан неверно."}), 400
    if new_norm == current_norm:
        payload = {"success": True, "phone": current_user.phone or "", "phone_masked": _mask_phone_for_ui(current_user.phone)}
        payload.update(_account_limits_payload(current_user))
        return jsonify(payload)
    if int(getattr(current_user, 'phone_change_count', 0) or 0) >= 2:
        return jsonify({"error": "Лимит перепривязки телефона исчерпан."}), 400
    existing = User.query.filter(User.phone == new_norm, User.id != current_user.id).first()
    if existing:
        return jsonify({"error": "Этот номер уже используется другим аккаунтом."}), 400
    try:
        current_user.phone = new_norm
        current_user.phone_change_count = int(getattr(current_user, 'phone_change_count', 0) or 0) + 1
        db.session.commit()
    except Exception:
        db.session.rollback()
        return jsonify({"error": "Не удалось сохранить."}), 500
...
# Register these only when a broken merge skipped the real auth routes.
...
if "recover_password" not in app.view_functions:
    app.add_url_rule("/recover", endpoint="recover_password", view_func=_recover_password_fallback, methods=["GET", "POST"])
if "forgot_password" not in app.view_functions:
    app.add_url_rule("/forgot", endpoint="forgot_password", view_func=_forgot_password_fallback, methods=["GET", "POST"])
if "reset_password" not in app.view_functions:
    app.add_url_rule("/reset/<token>", endpoint="reset_password", view_func=_reset_password_fallback, methods=["GET", "POST"])
...
def run_dev_server():
    ...
    # HTTPS is usually terminated by nginx/Beget in production.
    # Pass ssl_context only when it is really configured.
    ssl_ctx = None
    try:
        ...
        if cert and key and os.path.exists(cert) and os.path.exists(key):
            ssl_ctx = (cert, key)
        elif (os.environ.get("SSL_ADHOC") or "").strip().lower() in ("1", "true", "yes"):
            ssl_ctx = "adhoc"
    except Exception:
        ssl_ctx = None
...
# --- fix307 admin ip bans / delete restore / high-risk moderation ---
def _nc_admin_ipban_record(ip: str, reason: str = '', days: int = 3650):
    ip = (ip or '').strip()[:64]
    if not ip:
        return None
    now = datetime.utcnow()
    exp = now + timedelta(days=max(1, int(days or 3650)))
    row = AuthSecurityBlock.query.filter_by(scope_type='ip', scope_value=ip, phase='login', active=True).order_by(AuthSecurityBlock.id.desc()).first()
    if row:
        row.reason = reason or row.reason or 'admin_ip_ban'
        row.expires_at = exp
    else:
        row = AuthSecurityBlock(scope_type='ip', scope_value=ip, phase='login', reason=reason or 'admin_ip_ban', active=True, expires_at=exp)
        db.session.add(row)
    return row

def _nc_is_admin_ip_banned(ip: str) -> bool:
    ip = (ip or '').strip()
...
@app.before_request
def _nc_before_request_admin_ip_ban_guard():
    try:
        path = request.path or ''
        if path.startswith('/static/') or path.startswith('/favicon'):
            return None
        if current_user.is_authenticated and int(getattr(current_user,'id',0) or 0) == 1:
            return None
        ip = (_nc_client_ip() or '').strip()
        if not _nc_is_admin_ip_banned(ip):
            return None
        if path.startswith('/api/'):
            return jsonify(ok=False, error='ip_banned'), 403
        return render_template('simple_message.html', title='IP заблокирован', message='Доступ с этого IP-адреса заблокирован администратором.'), 403
    except Exception:
        return None
...
def api_admin_ip_ban():
    ...
    try:
        ...
        user_id = 0
    ip = (data.get('ip_address') or '').strip()[:64]
    if not ip and user_id:
        sess = AuthSession.query.filter_by(user_id=int(user_id)).order_by(AuthSession.last_seen_at.desc(), AuthSession.id.desc()).first()
        ip = (getattr(sess, 'ip_address', '') or '').strip()
        if not ip:
            att = AuthAttempt.query.filter_by(user_id=int(user_id)).order_by(AuthAttempt.created_at.desc(), AuthAttempt.id.desc()).first()
            ip = (getattr(att, 'ip_address', '') or '').strip()
    if not ip:
        return jsonify(ok=False, error='ip_required'), 400
    if ip in {'127.0.0.1','::1'}:
        return jsonify(ok=False, error='localhost_forbidden'), 400
    if action == 'ban':
        try:
            days = int(data.get('days') or 3650)
        except Exception:
            days = 3650
        _nc_admin_ipban_record(ip, reason or 'admin_ip_ban', days)
        db.session.commit()
        return jsonify(ok=True, action='ban', ip_address=ip)
    if action == 'unban':
        changed = 0
        for row in AuthSecurityBlock.query.filter_by(scope_type='ip', scope_value=ip, phase='login', active=True).all():
            row.active = False
            changed += 1
        db.session.commit()
        return jsonify(ok=True, action='unban', ip_address=ip, changed=changed)
    rows = AuthSecurityBlock.query.filter_by(scope_type='ip', phase='login', active=True).order_by(AuthSecurityBlock.id.desc()).limit(100).all()
    return jsonify(ok=True, items=[{'ip_address': r.scope_value, 'reason': r.reason or '', 'expires_at': _iso_z(getattr(r,'expires_at',None))} for r in rows])

@app.route('/api/admin/users/delete-mode', methods=['POST'])
@login_required
def api_admin_users_delete_mode():
    if not _nc_auth_is_admin_auditor('users'):
        return jsonify(ok=False, error='forbidden'), 403
    data = request.get_json(silent=True) or {}
    try:
    ...
    if not user:
        return jsonify(ok=False, error='not_found'), 404
    if int(user.id) == 1:
        return jsonify(ok=False, error='owner_forbidden'), 400
    if mode == 'soft_delete':
        user.is_deleted = True
        user.is_disabled = True
        _nc_admin_record_user_action(int(user.id), 'soft_delete', reason=reason, is_active=True)
        db.session.commit()
        _nc_admin_revoke_user_sessions(int(user.id))
        return jsonify(ok=True, mode=mode, user_id=int(user.id))
    if mode == 'purge_delete':
        uid = int(user.id)
        user.is_deleted = True
        user.is_disabled = True
        user.display_name = ''
        ...
        try: user.username_norm = _norm_username(user.username)
        except Exception: pass
        user.set_password(secrets.token_urlsafe(24))
        _nc_admin_record_user_action(int(user.id), 'purge_delete', reason=reason, is_active=True)
        db.session.commit()
        _nc_admin_revoke_user_sessions(int(user.id))
        return jsonify(ok=True, mode=mode, user_id=int(user.id))
    if mode == 'restore':
        ...
        user.is_disabled = False
        for row in AdminUserAction.query.filter(AdminUserAction.user_id==int(user.id), AdminUserAction.action_type.in_(['soft_delete']), AdminUserAction.is_active.is_(True)).all():
            row.is_active = False
        _nc_admin_record_user_action(int(user.id), 'restore_account', reason=reason, is_active=False)
        db.session.commit()
        return jsonify(ok=True, mode=mode, user_id=int(user.id))
    return jsonify(ok=False, error='bad_mode'), 400

@app.route('/admin/moderation/high-risk', methods=['GET'])
@login_required
def admin_moderation_high_risk():
    if not _nc_auth_is_admin_auditor('moderation'):
        abort(403)
    keywords = ['наркот','заклад','спайс','амф','меф','кокс','weed','drug','cocaine','amphetamine','mdma','марих','гаш','героин']
    conds = [MessageReport.reason.ilike('%drug%'), MessageReport.reason.ilike('%наркот%')]
    for kw in keywords:
        like = f'%{kw}%'
        conds.extend([MessageReport.details.ilike(like), Message.content.ilike(like)])
    rowsq = (db.session.query(MessageReport, Message, User, Channel)
        .outerjoin(Message, Message.id == MessageReport.message_id)
        .outerjoin(User, User.id == MessageReport.target_user_id)
        .outerjoin(Channel, Channel.id == MessageReport.channel_id)
        .filter(or_(*conds))
        .order_by(MessageReport.id.desc())
        .limit(200)
        .all())
    rows = []
    for rep, msg, user, ch in rowsq:
        rows.append({'id': int(rep.id), 'reason': rep.reason or 'high_risk', 'message_id': int(getattr(rep,'message_id',0) or 0), 'target_user_id': int(getattr(rep,'target_user_id',0) or 0) or None, 'channel_id': int(getattr(rep,'channel_id',0) or 0) or None, 'channel_name': getattr(ch,'name',None), 'username': getattr(user,'username',None), 'content': (getattr(msg,'content','') or getattr(rep,'details','') or '')[:500], 'created_at': getattr(rep,'created_at',None)})
    return render_template('admin_highrisk.html', rows=rows)

_old_api_account_delete = app.view_functions.get('api_account_delete')
if _old_api_account_delete:
    @app.route('/api/account/delete-soft', methods=['POST'])
    @login_required
    def api_account_delete_soft():
        data = request.get_json(silent=True) or {}
        pwd = (data.get('password') or '')
        confirm = (data.get('confirm') or '').strip().upper()
        if confirm not in {'DELETE','УДАЛИТЬ'}:
        ...
        return jsonify({'success': True, 'restorable': True})
    app.view_functions['api_account_delete'] = api_account_delete_soft
